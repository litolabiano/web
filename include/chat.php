...
<script>
    ...
    async function generateResponse(message) {
        try {
            // Build a reliable path to the site's `/web/` folder then call `/include/process.php` from there
            const path = window.location.pathname;
            const webIndex = path.indexOf('/web/');
            const base = webIndex !== -1 ? path.substring(0, webIndex + 5) : '/';
            const procUrl = base + 'include/process.php';

            const response = await fetch(procUrl, {
                method: 'POST',
